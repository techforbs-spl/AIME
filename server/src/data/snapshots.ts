...
const BASE_SNAPSHOTS: Record<string, DashboardSnapshot> = {
  allmax: {
...
export type SnapshotKey = keyof typeof BASE_SNAPSHOTS

export function getSnapshot(partnerParam?: string): DashboardSnapshot & { partner: string } {
  const key = (partnerParam ?? 'allmax').toLowerCase() as SnapshotKey
